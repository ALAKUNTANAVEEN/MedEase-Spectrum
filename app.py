...
from database import patient_login,admin_login, doctor_login,patient_reg,doctor_reg,add_symptoms,view_patient,dp_add,pp_act,res1,res_add,vr1,pvr1
...
def pvr():
    data=pvr1()
    print(data)
    return render_template("pvr.html", data=data)
...
@app.route("/resadd", methods = ['GET','POST'])
def resadd():
   if request.method == 'POST':      
      status = res_add(request.form['did'],request.form['doctor'],request.form['pid'],request.form['pname'],request.form['image'])
      if status == 1:
       return render_template("res.html",m1="sucess")
      else:
       return render_template("res.html",m1="failed") 
...
def upload():
      ...
      e=request.args.get('e')
      f=request.args.get('f')
      return render_template("res1.html",m1="sucess",did=a,doctor=b,pid=e,pname=f)
...
training_data_quesans = open('C:/Users/Naveen/OneDrive/Desktop/code/CBIT.txt').read().splitlines()
