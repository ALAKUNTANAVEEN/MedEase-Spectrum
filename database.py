...
def res_add(did, doctor, pid, pname, image):
        c, conn = db_connect()
        print(did, doctor, pid, pname, image)
        status = "pending"
        j = c.execute("insert into upload (did, doctor, pid, pname, image) values ('"+did+"','"+doctor+"','"+pid+"','"+pname+"','"+image+"')")
        conn.commit()
        conn.close()
        print(j)
        return j 
...
def res1():
        c, conn = db_connect()
        c.execute("select * from request where status = 'accepted' ")
        result=c.fetchall()
        conn.commit()
        conn.close()
        print(result)
...
def vr1():
        c, conn = db_connect()
        did = session['name']
        c.execute("select * from upload where doctor='"+did+"' ")
        result=c.fetchall()
        conn.commit()
        conn.close()
        print(result)
...
def pvr1():
        c, conn = db_connect()
        did = session['id']
        c.execute("select * from upload where pid='"+str(did)+"' ")
        result=c.fetchall()
        conn.commit()
        conn.close()
        print(result)
